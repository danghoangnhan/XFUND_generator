...
class XFUNDFormGenerator:
    ...
    def generate_xfund_from_template(
        ...
        output_image_path = os.path.join(output_dir, f"{template_name}_{self.annotation_id_counter:04d}.png")
    ...
    def _find_nearby_answers(
        ...
        for answer in answers:
            ...
            # Calculate distance
            distance = np.sqrt((q_center_x - a_center_x)**2 + (q_center_y - a_center_y)**2)
            
            if distance <= max_distance:
                nearby_answers.append(answer)
        
        # Sort by distance and return closest ones
        nearby_answers.sort(key=lambda a: np.sqrt(
...
def demo_xfund_form_generation():
    ...
    # Create generator
    generator = XFUNDFormGenerator(config)
    ...
    sample_annotations = [
        XFUNDAnnotation(
            ...
            text="Patient Name:",
            label="question",
            words=[Word(box=[50, 50, 95, 70], text="Patient"), Word(box=[100, 50, 150, 70], text="Name:")],
            linking=[[1, 2]]
        ),
        XFUNDAnnotation(
            id=2,
            ...
            text="John Doe", 
            label="answer",
            words=[Word(box=[160, 50, 195, 70], text="John"), Word(box=[200, 50, 250, 70], text="Doe")]
        )
    ]
