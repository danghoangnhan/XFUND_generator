...
class BBox:
    ...
    def normalize(self, img_width: int, img_height: int, target_size: int = 1000) -> 'BBox':
    ...
    def denormalize(self, img_width: int, img_height: int, source_size: int = 1000) -> 'BBox':
...
def split_text_bbox(text: str, bbox: Union[BBox, BBoxModel], add_jitter: bool = True) -> List[Tuple[str, Union[BBox, BBoxModel]]]:
    """
...
def split_text_into_words(text: str, bbox: 'BBoxModel', add_jitter: bool = True) -> List[Tuple[str, 'BBoxModel']]:
...
def select_random_font(fonts_dir: str, default_font_size: int = 12) -> Optional[ImageFont.FreeTypeFont]:
    """
...
def save_xfund_annotation_pydantic(
    ...
    """
    annotation_dict = {
        "form": [entity.model_dump() for entity in annotation.form]
    }
...
def validate_annotation_quality(
    ...
    for entity in annotation.form:
        ...
        if entity.bbox.area() < min_bbox_area:
            issues.append(f"Entity {entity.id} has bbox area too small: {entity.bbox.area()}")
...
FIELD_MAPPINGS = {
    'hospital_name': ['hospital name', 'hospital_name', 'clinic name', 'medical center'],
    ...
    'doctor_comment': ['doctor comment', 'doctor_comment', 'prescription', 'treatment', 'medication']
}
