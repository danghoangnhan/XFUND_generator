...
class XFUNDGenerator:
    ...
    def __init__(self, config: GeneratorConfig):
        ...
        logger.info(f"Initialized XFUND generator with Pydantic validation")
    ...
    def generate_dataset(self) -> GenerationResult:
        ...
        for i, data_record in enumerate(csv_records):
            try:
                ...
                entry_result = self._generate_single_entry_validated(data_record, template_info, i)
                
                if entry_result["success"]:
                    result.generated_entries += 1
                    logger.info(f"Generated validated entry {i+1}/{len(csv_records)}: {entry_result['entry_id']}")
                else:
                    result.add_error(f"Entry {i}: {entry_result.get('error', 'Unknown error')}")
                    logger.warning(f"Failed to generate entry {i+1}: {entry_result.get('error')}")
    ...
    def _find_templates(self) -> List[Dict[str, str]]:
        ...
        for docx_path in docx_files:
            ...
            if not template_validation["valid"]:
                logger.warning(f"Invalid template {docx_path}: {template_validation['error']}")
                continue
            ...
            if not layout_path.exists():
                ...
                logger.info(f"Creating basic layout for {template_name}. Please customize it!")
                
                # Create basic sample layout
                sample_layout = create_sample_layout()
                with open(layout_path, 'w') as f:
                    json.dump(sample_layout, f, indent=2)
                logger.info(f"Created {template_name}_layout.json - please review and adjust coordinates")
            ...
            try:
                ...
                required_fields = ["hospital_name", "hospital_address", "doctor_name", 
                                 "patient_name", "diagnose", "doctor_command"]
                missing_fields = [field for field in required_fields if field not in layout_data]
                
                if missing_fields:
                    logger.warning(f"Layout {layout_path} missing fields: {missing_fields}")
                else:
    ...
    def _generate_single_entry(
        ...
        try:
            ...
            if not validate_image_size(image_path):
                return {"success": False, "error": "Generated image too small or invalid"}
            ...
            if self.augmenter and self.config.get('enable_augmentations', True):
                ...
                # Apply augmentations
                augmented_image, augmented_annotations = self.augmenter.apply_augmentations(
                    image_rgb, annotations
                )
                
                # Validate augmentation quality
                aug_validation = validate_augmentation_quality(annotations, augmented_annotations)
                if not aug_validation["valid"] and self.config.get('strict_augmentation', False):
                    logger.warning(f"Augmentation quality issues for {entry_id}: {aug_validation['issues']}")
            ...
            if not validation_result["valid"] and self.config.get('strict_validation', False):
                return {
                    "success": False, 
            ...
            xfund_entry = renderer.create_xfund_entry(entry_id, image_filename, annotations)
    ...
    def generate_dataset_with_forms(self) -> GenerationResult:
        ...
        try:
            ...
            result.templates_used = len(set(r["template_name"] for r in successful))
            
            # Add errors from failed generations
            for failure in failed:
                result.add_error(f"Template {failure['template_name']}: {failure.get('error', 'Unknown error')}")
            
            # Log results
            logger.info(f"Form-based generation completed:")
...
def main():
    ...
    parser.add_argument(
        "--no-augmentations",
        action="store_true",
        help="Disable image augmentations"
    )
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Generate debug overlay images"
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
