...
class TestXFUNDFormIntegration:
    ...
    def test_xfund_form_generator_creation(self, sample_config):
        ...
        assert hasattr(generator, 'generate_xfund_dataset')

    @pytest.mark.integration
    @pytest.mark.forms
    @patch('src.xfund_form_integration.cv2', create=True)
    @patch('src.xfund_form_integration.Image', create=True)
    def test_generate_xfund_dataset(self, mock_image, mock_cv2, sample_config, temp_dir):
        """Test generating XFUND dataset from form data."""
        if not XFUND_INTEGRATION_AVAILABLE:
            pytest.skip("XFUND integration module not available")
            
        # Setup mocks
        mock_image.open.return_value.size = (1000, 800)
        mock_cv2.imread.return_value = MagicMock()
        mock_cv2.cvtColor.return_value = MagicMock()
        mock_cv2.boundingRect.return_value = (10, 20, 100, 80)
        
        # Create test data
        data_record = DataRecord(
            hospital_name_text="Test Hospital",
            doctor_name_text="Dr. Test"
        )
        ...
        test_image = temp_dir / "test_form.png"
        test_image.write_text("fake image content")
        
        # Generate dataset
        with patch.object(generator, '_detect_text_regions') as mock_detect:
            mock_detect.return_value = [
                {"text": "Hospital Name:", "bbox": [10, 20, 150, 40], "confidence": 0.9},
                {"text": "Test Hospital", "bbox": [160, 20, 300, 40], "confidence": 0.95}
            ]
            
            result = generator.generate_xfund_dataset(
                image_path=str(test_image),
                data_record=data_record
            )
        
        # Verify result
        assert isinstance(result, XFUNDDataset)
        assert result.image_path == str(test_image)
        assert len(result.annotations) > 0

    @pytest.mark.integration
    @pytest.mark.forms
    def test_question_answer_linking(self, sample_config):
        """Test automatic question-answer linking."""
        if not XFUND_INTEGRATION_AVAILABLE:
            pytest.skip("XFUND integration module not available")
        ...
        # Test Q&A pairing
        questions = ["Patient Name:", "Doctor Name:"]
        answers = ["John Doe", "Dr. Smith"]
        
        linked_pairs = generator._create_question_answer_pairs(questions, answers)
        
        assert len(linked_pairs) == 2
        assert linked_pairs[0]['question'] == "Patient Name:"
        assert linked_pairs[0]['answer'] == "John Doe"
    ...
    def test_medical_field_detection(self, sample_config):
        """Test medical field detection and classification."""
        generator = XFUNDFormGenerator(config=sample_config)
        
        # Test medical field classification
        test_cases = [
            ("Patient Name:", "question"),
            ("John Doe", "answer"),
            ("Hospital:", "question"),
            ("Central Hospital", "answer"),
            ("Random text", "other")
        ]
        
        for text, expected_label in test_cases:
            label = generator._classify_medical_text(text)
            assert label in ["question", "answer", "other"]
...
class TestFormGenerationPipeline:
    ...
    @pytest.mark.integration
    @pytest.mark.slow
    @patch('src.xfund_form_integration.cv2')
    @patch('src.xfund_form_integration.Image')
    def test_complete_pipeline(self, mock_image, mock_cv2, sample_config, temp_dir):
        ...
        # Create test files
        test_image = temp_dir / "medical_form.png"
        ...
        # Create data record
        data_record = DataRecord(
            hospital_name_text="General Hospital",
            doctor_name_text="Dr. Johnson",
            patient_name_text="Jane Smith"
        )
        ...
        with patch.object(generator, '_detect_text_regions') as mock_detect:
            mock_detect.return_value = [
                {"text": "Hospital Name:", "bbox": [10, 20, 150, 40]},
                {"text": "General Hospital", "bbox": [160, 20, 350, 40]},
                {"text": "Doctor:", "bbox": [10, 60, 80, 80]},
                {"text": "Dr. Johnson", "bbox": [90, 60, 200, 80]}
            ]
            
            # Generate dataset
            dataset = generator.generate_xfund_dataset(
                image_path=str(test_image),
                data_record=data_record
            )
    ...
    def test_error_handling_in_pipeline(self, sample_config):
        ...
        with pytest.raises((FileNotFoundError, OSError)):
            generator.generate_xfund_dataset(
                image_path="nonexistent_image.png",
                data_record=DataRecord(hospital_name_text="Test")
            )
    ...
    def test_configuration_validation_integration(self):
        ...
        try:
            ...
            assert generator.config.document_type.value == "medical"
    ...
    def test_migration_path_from_separate_methods(self, sample_datasets):
        ...
        for format_name, dataset in sample_datasets.items():
            ...
            if format_name == 'xfund':
                ...
                assert has_linking or all(ann.get('linking', []) == [] for ann in parsed['annotations'])
