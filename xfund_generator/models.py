...
class BBoxModel(BaseModel):
    ...
    def normalize(self, img_width: int, img_height: int, target_size: int = 1000) -> 'BBoxModel':
...
class XFUNDEntity(BaseModel):
    ...
    linking: Optional[List[List[int]]] = Field(default=None, description="Entity linking information")
...
class AugmentationConfig(BaseModel):
    ...
    brightness_range: tuple[float, float] = Field(default=(0.8, 1.2), description="Brightness adjustment range")
    contrast_range: tuple[float, float] = Field(default=(0.8, 1.2), description="Contrast adjustment range")
    blur_probability: float = Field(default=0.3, ge=0.0, le=1.0, description="Probability of applying blur")
    noise_probability: float = Field(default=0.2, ge=0.0, le=1.0, description="Probability of adding noise")
    rotation_range: tuple[float, float] = Field(default=(-2.0, 2.0), description="Rotation angle range in degrees")
    perspective_probability: float = Field(default=0.1, ge=0.0, le=1.0, description="Probability of perspective transform")
...
class TemplateConfig(BaseModel):
    ...
    fields: Dict[str, Any] = Field(default_factory=dict, description="Template field mappings")
...
class DataRecord(BaseModel):
    ...
    hospital_name_text: Optional[str] = Field(default="", description="Hospital name")
    hospital_address_text: Optional[str] = Field(default="", description="Hospital address")
    doctor_name_text: Optional[str] = Field(default="", description="Doctor name")
    department_text: Optional[str] = Field(default="", description="Medical department")
    diagnose_text: Optional[str] = Field(default="", description="Diagnosis")
    doctor_comment_text: Optional[str] = Field(default="", description="Doctor comments")
    
    # Additional fields for other document types
    additional_fields: Optional[Dict[str, str]] = Field(default_factory=dict, description="Additional custom fields")
...
class GeneratorConfig(BaseModel):
    ...
    # Optional paths
    fonts_dir: Optional[str] = Field(default=None, description="Directory containing custom fonts")
    
    # Image settings
    image_dpi: int = Field(default=300, ge=72, le=600, description="Image resolution in DPI")
    target_size: int = Field(default=1000, ge=224, description="Target size for XFUND normalization")
    
    # Generation settings
    document_type: DocumentType = Field(default=DocumentType.MEDICAL, description="Type of documents to generate")
    enable_augmentations: bool = Field(default=True, description="Enable data augmentations")
    augmentation_difficulty: AugmentationDifficulty = Field(default=AugmentationDifficulty.MEDIUM)
    
    # Quality settings
    add_bbox_jitter: bool = Field(default=True, description="Add small random jitter to bounding boxes")
    strict_validation: bool = Field(default=False, description="Enable strict validation")
    strict_augmentation: bool = Field(default=False, description="Enable strict augmentation validation")
    generate_debug_overlays: bool = Field(default=False, description="Generate debug overlay images")
    
    # Performance settings
    max_workers: int = Field(default=4, ge=1, description="Maximum number of worker processes")
...
class GenerationResult(BaseModel):
    ...
    generated_entries: int = Field(default=0, description="Number of successfully generated entries")
    ...
    output_paths: Dict[str, str] = Field(default_factory=dict, description="Paths to generated outputs")
...
class TemplateValidationResult(BaseModel):
    ...
    placeholders: List[str] = Field(default_factory=list, description="Found placeholders in template")
    paragraph_count: int = Field(0, ge=0, description="Number of paragraphs in template")
...
def create_xfund_entity(entity_id: int, text: str, bbox: BBoxModel, label: str) -> XFUNDEntity:
...
def validate_config_file(config_path: str) -> ValidationResult:
    ...
    try:
        ...
        config = GeneratorConfig.from_json_file(config_path)
        
        # Additional validation checks
        if not Path(config.templates_dir).exists():
            result.add_error(f"Templates directory does not exist: {config.templates_dir}")
        
        if not Path(config.csv_path).exists():
            result.add_error(f"CSV file does not exist: {config.csv_path}")
            
        if config.fonts_dir and not Path(config.fonts_dir).exists():
            result.add_warning(f"Fonts directory does not exist: {config.fonts_dir}")
